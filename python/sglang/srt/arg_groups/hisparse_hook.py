...
def validate_hisparse(server_args: ServerArgs) -> None:
    ...
    if use_runtime_sparse_attention(server_args):
        ...
        if not decode_default and decode_backend not in (
            Backend.BREAKABLE,
            Backend.DISABLED,
        ):
            raise ValueError(
                f"{subject} supports decode CUDA graph only with breakable backend."
            )
        ...
        if not prefill_default and prefill_backend != Backend.DISABLED:
            raise ValueError(f"{subject} requires prefill CUDA graph to be disabled.")
        return
...
def apply_runtime_sparse_cuda_graph_defaults(server_args: ServerArgs) -> None:
    """Apply graph defaults only when the user did not lock those settings."""
    ...
    if (
        ...
        logger.warning(
            "Quest uses breakable decode CUDA graph for query-dependent retrieval."
        )
    ...
    if (
        ...
        logger.warning(
            "Prefill CUDA graph is disabled because Quest builds page representations."
        )
        server_args.cuda_graph_config.prefill.backend = Backend.DISABLED
