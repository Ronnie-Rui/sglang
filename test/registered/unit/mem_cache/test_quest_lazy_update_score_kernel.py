...
class TestQuestLazyUpdateScoreKernel(unittest.TestCase):
    ...
    def test_tracker_only_update_uses_lazy_ready_boundary_and_grid_clamp(self):
        inputs = self._clone_inputs(self._make_inputs())
        inputs["seq_lens"].copy_(torch.tensor([17, 1, 10], device="cuda"))
        ...
        self.assertTrue(torch.all(inputs["repr_constructed"]).item())
        # Batch row 0 clamps ready page 4 to the three-page grid. Row 1 uses
        # (seq_len - 1) // page_size and must not move backwards; row 2 ignores
        # the stale value in an unconstructed tracker.
        self.assertTrue(
            torch.equal(
                inputs["last_constructed_page"],
